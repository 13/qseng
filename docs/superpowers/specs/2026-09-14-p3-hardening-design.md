...
| frontend | `actions/setup-node@v4` (22, npm cache), `npm ci` (in `frontend`), `npm run gates`, `npx ng lint`, `npx ng test --watch=false --coverage`, `npx ng build 2>&1 \| tee build.log` (under `set -o pipefail`) + `! grep -E "exceeded maximum budget" build.log`, artifact `web` (`dist/frontend/browser`) |
...
## 10. P3 follow-up B results (2026-09-15, branch `feat/p3-hardening`)

- Coverage widening, investigated: the installed `@angular/build` 21.2.8 unit-test builder's Vitest plugin rebuilds `test.coverage` from a fixed field whitelist (`enabled`/`include`/`exclude`/`thresholds`/`watermarks`/`reporter` — see `generateCoverageOption` in `node_modules/@angular/build/src/builders/unit-test/runners/vitest/plugins.js`) and silently drops every other key from a `runnerConfig` vitest.config.ts, so `coverage.all` cannot be threaded through this builder version; per the follow-up brief, that means stop rather than force a `vitest.config.ts` that would look like it does something and wouldn't. Before stopping, verified via `coverage/frontend/lcov.info` that the full `ng test --coverage` run already instruments all 24 non-spec, non-generated `core/`/`shared/` files with real (non-zero) hit counts — not just the 15 that have their own matching `*.spec.ts`. The other 9 (`auth.interceptor.ts`, `translate.pipe.ts`, `lang-preference.service.ts`, `form-errors.pipe.ts`, `server-errors.ts`, `breadcrumb.service.ts`, `pending-requests.service.ts`, `person-helpers.ts`, `partial-date.pipe.ts`) are loaded transitively by the rest of the suite, and v8 coverage only needs a module to execute during the run — not to have a dedicated spec — to be instrumented. So the P3b "15 of 24" caveat described which files have an own-name spec, not an actual coverage gap; there was never a gap to widen. The pinned thresholds (85/84/79/88) already equal the full 24-file measurement minus 5, floored, so no threshold change was needed either. No `vitest.config.ts` added.
- Frontend production bundle (`ng build`): initial total 793.04 kB, no bundle-budget violation (the CI build step tees the log under `pipefail` and greps for `exceeded maximum budget`).
