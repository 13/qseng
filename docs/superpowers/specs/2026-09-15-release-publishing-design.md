...
4. Compute version inputs in a small `run` step exporting to `$GITHUB_OUTPUT`:
   - Tag validation: when `$GITHUB_REF` is a tag, `${GITHUB_REF_NAME}` must match `^v[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$`; otherwise `echo "::error::tag must look like vX.Y.Z or vX.Y.Z-pre"` and `exit 1` before any output is written, so a mistyped or non-semver tag (`v1`) fails the job instead of silently publishing `latest` with no matching version tag.
   ...
   - `stable`: `true`/`false`, derived from whether `version` contains `-`; `false` off a tag (`main`/PRs never publish `latest`). Drives the `latest` tag's `enable=` expression below.
   - `commit`: `${{ github.sha }}`.
...
6. `docker/build-push-action@v6`: `context: .`, `file: docker/Dockerfile.${{ matrix.image }}`, `platforms: linux/amd64`, `push: ${{ github.event_name == 'push' }}`, `tags`/`labels` from step 5, `build-args: QSENG_VERSION=…, QSENG_COMMIT=…`, `cache-from: type=gha,scope=${{ matrix.image }}`, `cache-to: type=gha,mode=max,scope=${{ matrix.image }}`. Output `digest`. **Deliberate deviation:** `load: ${{ github.event_name != 'push' }}` from an earlier draft of this spec was dropped — nothing in the job consumes a locally-loaded image on a PR run (the build itself still validates the Dockerfile), so `load:` is not used on PR builds.
...
**Job `release`.** `if: startsWith(github.ref, 'refs/tags/v')`, `needs: [images]`, `permissions: { contents: write }`, `timeout-minutes: 5`. Steps: checkout; download the two `image-*.txt` artifacts; build the Images block (image refs with digests + the attestation verify commands, owner lowercased for both the image path and `--owner`); fetch GitHub's generated notes explicitly via `gh api "repos/$GITHUB_REPOSITORY/releases/generate-notes" -f tag_name="$TAG" -q .body`, then compose the final body as generated notes, a blank line, then the Images block — the same order and the same explicit composition on both the create and the edit path (not `--generate-notes`, which only exists on `create` and would let a re-run's `gh release edit` overwrite the notes with the Images block alone). Create: `gh release create "$TAG" --verify-tag --notes-file body.md`, plus `--prerelease` when the tag's version contains `-`. If a Release for the tag already exists (re-run), `gh release edit "$TAG" --notes-file body.md --prerelease=<true|false>` instead of failing — an edit always states `--prerelease` explicitly so a re-run can flip it if needed, rather than leaving a stale value from an earlier run.
